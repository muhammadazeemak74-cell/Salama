...
def phase1_bh_table(base: pd.DataFrame, costs: CostModel) -> pd.DataFrame:
    ...
    for tf in gdata.TIMEFRAMES:
        ...
        _, diag = run(bars, Config("green", tf, 3, "S1", 1.0), costs)
        avg_spread = float(
...
def phase2_grid(bars_by_tf: dict, costs: CostModel, polarity: str) -> pd.DataFrame:
    rows = []
    for cfg in grid(polarity):
        trades, diag = run(bars_by_tf[cfg.timeframe], cfg, costs)
...
def main() -> int:
    ...
    frames = [phase2_grid(is_by_tf, costs, p) for p in ("green", "red")]
    ...
    oos_trades, _ = run(oos_bars, cfg, costs)
    ...
    write_verdict(bh, is_df, nom, {"net": oos_net, "gross": oos_gross}, costs, split)
    return 0
